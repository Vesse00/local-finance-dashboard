...
export function MultiExpenseForm({ selectedDate, categories, onSuccess, currency = "PLN" }: MultiExpenseFormProps) {
  ...
  const getSubmitLabel = () => {
    if (isPending) return t("calendar.modals.expense.multi_submitting");
    if (filledRows.length === 0) return t("calendar.modals.expense.multi_submit_one");
    if (language === "pl") {
      if (filledRows.length === 1) return t("calendar.modals.expense.multi_submit_one");
      if (filledRows.length <= 4) return t("calendar.modals.expense.multi_submit_few").replace("{count}", String(filledRows.length));
      return t("calendar.modals.expense.multi_submit_many").replace("{count}", String(filledRows.length));
    }
    return t("calendar.modals.expense.multi_submit_few").replace("{count}", String(filledRows.length));
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="animate-in fade-in duration-300">
      ...
      <div className="mb-4 flex items-center gap-2 text-xs text-zinc-500 dark:text-zinc-400 bg-black/5 dark:bg-white/5 px-3 py-2 rounded-xl">
        ...
        <span>{t("calendar.modals.expense.multi_hint")}</span>
      </div>

      {/* NAGŁÓWEK KOLUMN */}
      <div className="grid grid-cols-[130px_1fr_170px_36px] gap-2 mb-1.5 px-1">
        <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">{t("calendar.modals.expense.multi_col_amount")}</span>
        <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">{t("calendar.modals.expense.multi_col_description")}</span>
        <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">{t("calendar.modals.expense.multi_col_category")}</span>
        <span />
      </div>
        ...
        {rows.map((row, idx) => {
          ...
          return (
            ...
            >
              ...
              <input
                type="text"
                placeholder={t("calendar.modals.expense.multi_desc_placeholder")}
        ...
        <div className="text-sm">
          {filledRows.length > 0 ? (
            <span className="text-zinc-600 dark:text-zinc-400">
              <span className="font-bold text-zinc-800 dark:text-zinc-200">{filledRows.length}</span>
              {" · "}
              {t("calendar.modals.expense.multi_summary_total")}{" "}
              <span className="font-bold text-primary">
                {total.toLocaleString(language === "pl" ? "pl-PL" : "en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} {currency}
              </span>
            </span>
          ) : (
            <span className="text-zinc-400 text-xs">{t("calendar.modals.expense.multi_empty_rows")}</span>
          )}
        </div>
        <button
          type="submit"
