...
export default async function SavingsPage() {
  // Pobieramy usera (Później podepniesz autoryzację)
  const user = await prisma.user.findFirst();
  if (!user) return null;

  // Pobieramy historię transferów na oszczędności
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 p-8 rounded-3xl border border-blue-500/20 bg-blue-500/5 backdrop-blur-xl relative overflow-hidden">
        ...
        <div>
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400 max-w-sm mt-2">
            Pieniądze przeniesione z konta głównego, odłożona "reszta" z miesiąca oraz saldo kont oszczędnościowych z importu.
          </p>
        </div>
      ...
      <div>
        ...
        <div className="space-y-3">
          {savingHistory.length === 0 ? (
            <div className="text-center py-8 text-zinc-500">Jeszcze nic nie odłożono.</div>
          ) : (
