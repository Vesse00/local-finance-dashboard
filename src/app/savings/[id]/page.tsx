...
export default function SavingsDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading || !account) return <div className="p-12 text-center text-zinc-500 animate-pulse">Ładowanie historii...</div>;
  ...
  return (
    <div className="flex-1 p-6 md:p-8 space-y-6 max-w-4xl mx-auto w-full">
      <Link href="/savings" className="inline-flex items-center gap-2 text-sm font-bold text-zinc-500 hover:text-zinc-900 dark:hover:text-white transition-colors">
        <ArrowLeft className="w-4 h-4" /> Wróć do Centrum Oszczędności
      </Link>
      ...
      <div className={`bg-gradient-to-br from-${theme}-500/20 to-${theme}-500/5 border border-${theme}-500/30 backdrop-blur-xl rounded-3xl p-8 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6`}>
        ...
        <div className="text-center md:text-right">
          ...
          >
            <Scale className="w-4 h-4" /> Korekta Salda
          </button>
        </div>
      </div>
        ...
        <div className="space-y-2">
          {(!account.history || account.history.length === 0) ? (
            <div className="text-center py-10 text-zinc-500 text-sm border border-dashed border-zinc-200 dark:border-zinc-800 rounded-2xl bg-zinc-50 dark:bg-zinc-900/50">
              Brak historii. Wpłać pieniądze w kalendarzu lub zrób pierwszą korektę salda.
            </div>
          ) : (
              ...
              <div key={tx.id} className="flex items-center justify-between p-4 rounded-2xl border border-black/5 dark:border-white/5 bg-white dark:bg-zinc-900/50 hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors group">
                <div className="flex items-center gap-4 overflow-hidden">
                  ...
                  <div className="min-w-0">
                    <p className="font-bold text-sm text-zinc-900 dark:text-white truncate">{tx.description || (tx.type === "IN" ? "Wpłata" : "Wypłata")}</p>
                    <p className="text-xs font-medium text-zinc-500 mt-0.5">{format(new Date(tx.date), "dd MMMM yyyy, HH:mm", { locale: pl })}</p>
                  </div>
                </div>
          ...
          <div className="bg-white dark:bg-zinc-950 w-full max-w-md rounded-3xl shadow-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden animate-in zoom-in-95 flex flex-col">
            <div className="p-6 border-b border-zinc-100 dark:border-zinc-900 flex justify-between items-center bg-zinc-50/50 dark:bg-zinc-900/20">
              <h3 className="font-bold text-lg flex items-center gap-2">
                <Scale className="w-5 h-5 text-indigo-500" /> Korekta Salda
              </h3>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-zinc-500 uppercase flex items-center gap-1"><Calculator className="w-3.5 h-3.5 text-indigo-500"/> Zaktualizuj kwotę (Stan z Banku)</label>
                  ...
                  <div className="space-y-2 w-full">
                    ...
                    <div className="space-y-1">
                      ...
                      >
                        {isDiffPositive ? (
                          <>
                            <option value="PROFIT">📈 Zysk z inwestycji (Wzrost)</option>
                            <option value="INTEREST">💰 Kapitalizacja Odsetek (Lokata)</option>
                            <option value="MANUAL">✏️ Inna korekta na plus</option>
                          </>
                        ) : (
                          <>
                            <option value="LOSS">📉 Strata z inwestycji (Spadek)</option>
                            <option value="FEE">💸 Prowizja / Opłata za konto</option>
                            <option value="MANUAL">✏️ Inna korekta na minus</option>
                          </>
                        )}
                      </select>
                    </div>
            ...
            <div className="p-4 bg-zinc-50 dark:bg-zinc-900/50 flex gap-3 border-t border-zinc-100 dark:border-zinc-900">
              <button onClick={() => setIsCorrectionModalOpen(false)} className="flex-1 py-3 px-4 rounded-xl font-bold border border-zinc-200 dark:border-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors">Anuluj</button>
              <button 
              ...
              >
                {isSaving ? "Zapisywanie..." : "Zastosuj korektę"}
              </button>
            </div>
          </div>
        </div>
